...
String palms_cascade_name = "CascadeFiles/haarcascade_palm.xml";
String hands_cascade_name = "CascadeFiles/haarcascade_hand.xml";

// Face detection using Haar-like features
void detectHand( Mat frame )
{
    // Load the cascade
    CascadeClassifier hands_cascade;
    if( !hands_cascade.load( hands_cascade_name ) ) { cout << "Error loading hand cascade file\n" << endl; return; };

    CascadeClassifier palms_cascade;
    if( !palms_cascade.load( palms_cascade_name ) ) { cout << "Error loading palm cascade file\n" << endl; return; };
    ...
    // Detect hands and palms
    hands_cascade.detectMultiScale( frame_gray, hands, 1.1, 2, 0|CV_HAAR_SCALE_IMAGE, Size(30, 30) );
    palms_cascade.detectMultiScale( frame_gray, palms, 1.1, 2, 0|CV_HAAR_SCALE_IMAGE, Size(30, 30) );
    
    for( size_t i = 0; i < palms.size(); i++ ){
	hands.push_back(palms[i]);
    }
    
    for( size_t i = 0; i < hands.size(); i++ )
    {
        Point center( hands[i].x + hands[i].width*0.5, hands[i].y + hands[i].height*0.5 );
        ellipse( frame, center, Size( hands[i].width*0.5, hands[i].height*0.5), 0, 0, 360, Scalar( 255, 255, 255 ), 4, 8, 0 );
    }
